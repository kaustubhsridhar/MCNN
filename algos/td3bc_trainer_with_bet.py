...
from offlinerlkit.buffer import SequenceReplayBuffer
from offlinerlkit.utils.logger import Logger, make_log_dirs_td3bc
from offlinerlkit.policy_trainer import MFPolicyTrainer
from offlinerlkit.policy import BetTD3BCPolicy, MemBetTD3BCPolicy
from miniBET.behavior_transformer import BehaviorTransformer, GPT, GPTConfig
...
def get_args():
    ...
    parser.add_argument("--algo-name", type=str, default="bet", choices=["mem_bet", "bet"])
    parser.add_argument("--task", type=str, default="pen-human-v1")
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--actor-lr", type=float, default=5.5e-5) # used to be 3e-4 for MLP
...
def train(args=get_args()):
    # update args
    if args.algo_name in ["bc", "mem_bc", "bet", "mem_bet"]:
        args.only_bc = True
        args.alpha = 0.0
    else:
        args.only_bc = False
    ...
    # create policy model
    conditional = False
    goal_dim = 0 if not conditional else 60
    actor = BehaviorTransformer(
                        ...
                        n_clusters=64,  # Number of clusters to use for k-means discretization.
                        kmeans_fit_steps=100,  # The k-means discretization is done on the actions seen in the first kmeans_fit_steps.
                        Lipz=args.Lipz if "mem" in args.algo_name else None, # for mem_bet only
                        lamda=args.lamda if "mem" in args.algo_name else None, # for mem_bet only
                    ).to(args.device)
    actor_optim = actor.configure_optimizers(
        weight_decay=2e-4,
    ...
    policy_trainer = MFPolicyTrainer(
        ...
        sequential=True if 'bet' in args.algo_name else False,
    )
