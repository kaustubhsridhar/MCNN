...
class DeterministicMemEnsembleDynamics(BaseDynamics):
    def __init__(
        self,
        model: nn.Module,
        ...
        uncertainty_mode: str = "dist_to_memory",
        dataset: Dict = {},
        Lipz: float = 100.0,
        lamda: float = 1.0,
        train_horizon: int = 1,
        ...
        self.ensemble_memories_inputs = self.memories_inputs.unsqueeze(0).repeat(self.model.num_ensemble, 1, 1)
    ...
    def find_memories(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        if inputs.ndim == 2:
            ...
            _, closest_nodes = torch.cdist(inputs, self.ensemble_memories_inputs).min(dim=-1)
            mem_inputs = []
            mem_targets = []
            for en_idx in range(inputs.shape[0]):
                mem_obss = self.memories_obs[closest_nodes[en_idx], :]
                mem_actions = self.memories_actions[closest_nodes[en_idx], :]
                mem_next_obss = self.memories_next_obs[closest_nodes[en_idx], :]
                mem_rewards = self.memories_rewards[closest_nodes[en_idx], :]

                delta_mem_obss = (mem_next_obss - mem_obss) / self.obss_abs_max_tensor

                this_mem_inputs = torch.cat([mem_obss, mem_actions], dim=-1)
                this_mem_targets = torch.cat([delta_mem_obss, mem_rewards], dim=-1)

                mem_inputs.append(this_mem_inputs)
                mem_targets.append(this_mem_targets)
            
            mem_inputs = torch.stack(mem_inputs, dim=0)
            mem_targets = torch.stack(mem_targets, dim=0)
    ...
    def step(
        ...
        ensemble_samples = mean.astype(np.float32) # (mean + np.random.normal(size=mean.shape) * std).astype(np.float32)
        ...
        if self._penalty_coef:
            if self._uncertainty_mode == "pairwise-diff":
            ...
            elif self._uncertainty_mode == 'dist_to_memory':
                penalty = -1.0 / dist
                print(dist.shape, penalty.shape, reward.shape)
                exit()
            else:
                raise ValueError
    ...
    def learn(
        ...
        for batch_num in range(int(np.ceil(train_size / batch_size))):
            ...
            first_mem_inputs_batch = mem_inputs[:, batch_num * batch_size:(batch_num + 1) * batch_size]
            first_mem_inputs_batch = torch.as_tensor(first_mem_inputs_batch).to(self.model.device)
            
            list_of_targets_batch = [targets[:, (batch_num) * batch_size + T - 1:(batch_num + 1) * batch_size + T - 1] for T in range(1, H+1)]
            list_of_targets_batch = [torch.as_tensor(item).to(self.model.device) for item in list_of_targets_batch]
            
            first_mem_targets_batch = mem_targets[:, batch_num * batch_size:(batch_num + 1) * batch_size]
            first_mem_targets_batch = torch.as_tensor(first_mem_targets_batch).to(self.model.device)
            
            loss = 0.0
            for target_idx, targets_batch in enumerate(list_of_targets_batch):
                raw_mean = self.model(inputs_batch)

                if target_idx == 0:
                    mem_inputs_batch = first_mem_inputs_batch
                    mem_targets_batch = first_mem_targets_batch
                else:
                    mem_inputs_batch, mem_targets_batch = self.find_memories(inputs_batch)
                ...
                inv_var_matrix = 1.0 
                mse_loss_inv = (torch.pow(mean - targets_batch, 2) * inv_var_matrix).mean(dim=(1, 2))
                loss = loss + mse_loss_inv.sum() 
