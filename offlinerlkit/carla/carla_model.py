...
from offlinerlkit.carla.carla_building_blocks import Conv, Branching, FC, Join, resnet34

from offlinerlkit.carla.carla_config import SENSORS, TARGETS, INPUTS, PRE_TRAINED, NUMBER_FRAMES_FUSION
...
class CoILICRA(nn.Module):
    ...
    def __init__(self, params):
        ...
        for _, sizes in SENSORS.items():
            number_first_layer_channels += sizes[0] * NUMBER_FRAMES_FUSION

        # Get one item from the dict
        sensor_input_shape = next(iter(SENSORS.values()))
        ...
        if 'conv' in params['perception']:
            ...
            self.perception = resnet_module(pretrained=PRE_TRAINED,
        ...
        self.measurements = FC(params={'neurons': [len(INPUTS)] +
