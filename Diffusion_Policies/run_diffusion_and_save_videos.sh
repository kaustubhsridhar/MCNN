...
do
    ...
    do
        ...
        do
            ...
            do
                CUDA_VISIBLE_DEVICES=${GPU} nohup python -u main.py --seed ${SEED} --algo mcnn_bc --env_name ${task} --device 0 --ms online --lr_decay --num_memories_frac ${F} --Lipz 1.0 --lamda ${lamda} --save_videos > logs/mcnn_bc_${task}_F${F}_lamda${lamda}_with_saved_videos.log &
            done
        done
    done
    GPU=$((GPU+1))
