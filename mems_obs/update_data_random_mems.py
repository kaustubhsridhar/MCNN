...
for chosen_percentage in [1.0]: # 0.1, 0.2, 0.5, 1.0
    ...
    NEW = 'random' # for random memories
    save_name = f'{folder}/{full_name}/{NEW}_updated_{args.num_memories_frac}_frac.pkl'
    ...
    if not os.path.exists(save_name):
        ...
        # load random subset of all observations
        t0 = time.time()
        random_indices = np.random.choice(all_observations.shape[0], int(args.num_memories_frac * all_observations.shape[0]), replace=False)
        memories = torch.from_numpy(all_observations[random_indices]).float().to(device)
        memories_actions = torch.from_numpy(all_actions[random_indices]).float().to(device)
        memories_next_obs = torch.from_numpy(all_next_observations[random_indices]).float().to(device)
        memories_rewards = torch.from_numpy(all_rewards[random_indices]).float().to(device)
        print(f'{memories.shape=}, {memories_actions.shape=}, {memories_next_obs.shape=}, {memories_rewards.shape=}')
        print(f'Choosing random memories took {time.time() - t0} seconds')
